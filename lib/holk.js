...
/*
 * credentialsFunc    - required function to lookup the set of Holk credentials based on the provided credentials id.
...
exports.getWWWAuthenticateHeader = function (message) {
    ...
    return 'Holk' + (message ? ' error="' + message + '"' : '');
};
...
exports.getAuthorizationHeader = function (credentials, method, uri, host, port, ext) {
    ...
    var header = 'Holk id="' + credentials.id +
                 '", ts="' + timestamp +
                 '", nonce="' + nonce +
                 (ext ? '", ext="' + ext : '') +
                 '", mac="' + mac + '"';
